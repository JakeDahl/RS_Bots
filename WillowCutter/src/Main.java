...
@ScriptManifest(name = "Willow Cutter", description = "Cuts willows in Draynor Village with Dragon axe and banks logs",
        author = "WillowCutter", version = 1.0, category = Category.WOODCUTTING, image = "")

public class Main extends AbstractScript {
    
    // Draynor Village willow trees area (near the bank)
    private final Tile WILLOW_TREE_TILE = new Tile(3088, 3233); // Draynor willow trees
    private final Area WILLOW_AREA = WILLOW_TREE_TILE.getArea(10);
    
    // Draynor Village bank
    private final Tile DRAYNOR_BANK_TILE = new Tile(3092, 3243);
    private final Area DRAYNOR_BANK_AREA = DRAYNOR_BANK_TILE.getArea(5);
    ...
    @Override
    public void onStart() {
        Logger.log("Willow Cutter started!");
        Logger.log("Target: Willow trees in Draynor Village");
        Logger.log("Banking at: Draynor Village Bank");
        ...
        if (!hasRequiredAxe()) {
            ...
            Logger.log("Dragon axe detected - ready to cut!");
        }
    }
    ...
    private State getState() {
        ...
        if (Inventory.isFull()) {
            ...
            if (WILLOW_AREA.contains(Players.getLocal())) {
                if (isPlayerCutting()) {
                    return State.IDLE; // Wait while cutting
                } else {
                    return State.CUTTING_WILLOWS;
                }
            } else {
    ...
    private boolean isNearBank() {
        return DRAYNOR_BANK_AREA.contains(Players.getLocal());
    }
    
    private void walkToWillowTrees() {
        if (!WILLOW_AREA.contains(Players.getLocal())) {
            Logger.log("Walking to willow trees in Draynor Village...");
            Tile randomizedTile = WILLOW_TREE_TILE.getRandomized(3);
            if (Walking.walk(randomizedTile)) {
                Sleep.sleepUntil(() -> WILLOW_AREA.contains(Players.getLocal()) || 
    ...
    private void cutWillowTree() {
        ...
        GameObject willowTree = GameObjects.closest("Willow tree");
        if (willowTree != null && willowTree.exists()) {
            Logger.log("Cutting willow tree...");
            if (willowTree.interact("Chop down")) {
                Sleep.sleepUntil(() -> isPlayerCutting() || 
                               Inventory.count("Willow logs") > getCurrentLogCount(), 
                               randomSleep(3000, 5000));
            }
        } else {
            Logger.log("No willow trees found nearby, waiting for respawn...");
    ...
    private int getCurrentLogCount() {
        return Inventory.count("Willow logs");
    }
    
    private void walkToBank() {
        Logger.log("Walking to Draynor Village bank...");
        Tile randomizedBankTile = DRAYNOR_BANK_TILE.getRandomized(2);
        if (Walking.walk(randomizedBankTile)) {
            Sleep.sleepUntil(() -> DRAYNOR_BANK_AREA.contains(Players.getLocal()) || 
                           !Players.getLocal().isMoving(), 
                           randomSleep(5000, 8000));
        }
    }
    ...
    private void handleBanking() {
        ...
        if (Bank.isOpen()) {
            ...
            int logsToDeposit = Inventory.count("Willow logs");
            ...
            if (Bank.depositAllExcept("Dragon axe")) {
                ...
                // Fallback: deposit willow logs specifically if depositAllExcept fails
                int logCount = Inventory.count("Willow logs");
                if (logCount > 0) {
                    Logger.log("Fallback: Depositing " + logCount + " willow logs...");
                    if (Bank.deposit("Willow logs", logCount)) {
                        Sleep.sleepUntil(() -> !Inventory.contains("Willow logs"), 
                                       randomSleep(2000, 3000));
                        Logger.log("Successfully deposited willow logs");
            ...
            Logger.log("Banking complete, returning to willow trees");
        }
    }
    ...
    public int onLoop() {
        ...
        if (WILLOW_AREA.contains(Players.getLocal()) && !Inventory.isFull() && 
        ...
        switch (currentState) {
            ...
            case IDLE:
                ...
                Sleep.sleepUntil(() -> !isPlayerCutting() || Inventory.isFull() || 
                               getCurrentLogCount() != Inventory.count("Willow logs"), 
    ...
    @Override
    public void onExit() {
        Logger.log("Willow Cutter stopped!");
        Logger.log("Final log count in inventory: " + Inventory.count("Willow logs"));
        Logger.log("Total willow logs cut: " + logsCount);
    ...
    public void onPaint(Graphics2D graphics) {
        ...
        String title = "Willow Cutter - Draynor (Anti-PK)";
        ...
        yOffset += 15;
        graphics.drawString("Willow Logs Cut: " + numberFormat.format(logsCount), 20, yOffset);
        yOffset += 15;
